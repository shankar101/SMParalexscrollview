...
#import "BSSocialCenterButton.h"
#define SYSTEM_VERSION [[[UIDevice currentDevice] systemVersion] floatValue]

@interface BSSocialButton ()<BSSocialCenterButtonDelegate, BSSocialItemButtonDelegate>
...
@property (strong, nonatomic) BSSocialCenterButton *pathCenterButton;
...
@implementation BSSocialButton
@synthesize _delegateItemBtn;
...
- (void)configureViewsLayout
{
    ...
    else
    {
        if(SYSTEM_VERSION<8.0)
            self.bloomSize = CGSizeMake([UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width);
        else
            self.bloomSize = [UIScreen mainScreen].bounds.size;
    }
    self.bloom = NO;
    ...
    //
    self.foldCenter = CGPointMake(self.frame.size.width/2,self.frame.size.height/2);//CGPointMake(self.bloomSize.width / 2, self.bloomSize.height - 25.5f);
    self.bloomCenter = CGPointMake(self.frame.size.width/2,self.frame.size.height/2);//CGPointMake(self.bloomSize.width / 2, self.bloomSize.height / 2);
    ...
    //
    _pathCenterButton = [[BSSocialCenterButton alloc]initWithImage:self.centerImage highlightedImage:self.centerHighlightedImage];
...
- (CGPoint)createEndPointWithRadius:(CGFloat)itemExpandRadius andAngel:(CGFloat)angel withpointNo:(int)_no
{
    if([_str_forwhich isEqualToString:@"MygroupDtls"]){
        return CGPointMake(self.pathCenterButtonBloomCenter.x+40 - sinf(angel * M_PI) * ((itemExpandRadius/5)*4),
                       self.pathCenterButtonBloomCenter.y - cosf(angel * M_PI) * ((itemExpandRadius/5)*4));
    }else
    {
        if(_no==1)
        {
            return CGPointMake(90+self.pathCenterButtonBloomCenter.x - sinf(angel * M_PI) * itemExpandRadius,
                               40+self.pathCenterButtonBloomCenter.y - cosf(angel * M_PI) * itemExpandRadius);
        }else if (_no==2){
            return CGPointMake(70+self.pathCenterButtonBloomCenter.x - sinf(angel * M_PI) * itemExpandRadius,
                               5+self.pathCenterButtonBloomCenter.y - cosf(angel * M_PI) * itemExpandRadius);
        }else if (_no==3){
            return CGPointMake(70+self.pathCenterButtonBloomCenter.x - sinf(angel * M_PI) * itemExpandRadius,
                               self.pathCenterButtonBloomCenter.y - cosf(angel * M_PI) * itemExpandRadius);
        }else{
            return CGPointMake(90+self.pathCenterButtonBloomCenter.x - sinf(angel * M_PI) * itemExpandRadius,
                               self.pathCenterButtonBloomCenter.y-45 - cosf(angel * M_PI) * itemExpandRadius);
        }
...
- (void)pathCenterButtonFold
{
    ...
    for (int i = 1; i <= self.itemButtons.count; i++) {
        ...
        BSSocialItemButton *itemButton = self.itemButtons[i - 1];
        
        CGFloat currentAngel = i / ((CGFloat)self.itemButtons.count + 1);
        CGPoint farPoint = [self createEndPointWithRadius:self.bloomRadius + 5.0f andAngel:currentAngel withpointNo:i];
...
{
    [UIView animateWithDuration:0.0618f * 6
                          delay:0.0618f * 2
...
- (void)pathCenterButtonBloom
{
    ...
    for (int i = 1; i <= self.itemButtons.count; i++) {
        ...
        BSSocialItemButton *pathItemButton = self.itemButtons[i - 1];
        ...
        //
        CGPoint endPoint = [self createEndPointWithRadius:self.bloomRadius andAngel:currentAngel withpointNo:i];
        CGPoint farPoint = [self createEndPointWithRadius:self.bloomRadius + 10.0f andAngel:currentAngel withpointNo:i];
        CGPoint nearPoint = [self createEndPointWithRadius:self.bloomRadius - 5.0f andAngel:currentAngel withpointNo:i];
...
- (void)itemButtonTapped:(BSSocialItemButton *)itemButton
{
    if ([_delegate respondsToSelector:@selector(itemButtonTappedAtIndex:)]) {
        BSSocialItemButton *selectedButton = self.itemButtons[itemButton.tag];
        // Play selected sound
        //
        ...
        for (int i = 0; i < self.itemButtons.count; i++) {
            ...
            BSSocialItemButton *unselectedButton = self.itemButtons[i];
    ...
    [_delegateItemBtn MenuButttontap:(int)itemButton.tag];
